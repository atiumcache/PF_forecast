...
### Compute BIC of a pairwise cubic spline (PCS) model fit under AR(1) and changepoints
fit.PCSar_BIC <- function( y, cp ) {               # y    : data
                                                   # cp   : changepoint chromosome (m; tau_1,...,tau_m)
  n <- length( y )                                 # n    : sample size
  m <- cp[1]                                       # m    : number of changepoints
  tau <- cp[-1]                                    # tau  : time locations of changepoints
  ...
  if ( m==0 ) {
    ar.PCS <- arima( y,order=c(1,0,0),xreg=cbind(t,t2,t3),include.mean=TRUE,method="CSS-ML" )
  } else {
    D.m <- matrix( 0, nrow=n, ncol=m )
    for ( k in 1:m ) {
      D.m[,k] <- pmax( (t-tau[k])^3, 0 )
    }
    ar.PCS <- arima( y,order=c(1,0,0),xreg=cbind(t,t2,t3,D.m),include.mean=TRUE,method="CSS-ML" )
  }
...
### Fit a pairwise cubic spline (PCS) model under AR(1) and changepoints
fit.PCSar <- function( y, cp ) {                   # y    : data
                                                   # cp   : changepoint chromosome (m; tau_1,...,tau_m)
  n <- length( y )                                 # n    : sample size
  m <- cp[1]                                       # m    : number of changepoints
  tau <- cp[-1]                                    # tau  : time locations of changepoints
...
### Find the estimated trends in a PLT model under AR(1) and changepoints
fit.PCSar_trends <- function( y, cp ) {            # y    : data
                                                   # cp   : changepoint chromosome (m; tau_1,...,tau_m)
  n <- length( y )                                 # n    : sample size
  m <- cp[1]                                       # m    : number of changepoints
  tau <- cp[-1]                                    # tau  : time locations of changepoints
  ...
  if ( m==0 ) {
    ar.PCS <- arima( y,order=c(1,0,0),xreg=cbind(t,t2,t3),include.mean=TRUE,method="CSS-ML" )
    coef_reg <- coef( ar.PCS )[-1]                 # remove AR(1) parameter
    y_trd <- coef_reg[1] + coef_reg[2]*t + coef_reg[3]*t^2 + coef_reg[4]*t^3
  } else {
    D.m <- matrix( 0, nrow=n, ncol=m )
    for ( k in 1:m ) {
      D.m[,k] <- pmax( (t-tau[k])^3, 0 )
    ...
    y_trd.m <- rep(0,times=n)
    for ( k in 1:m ) {
      y_trd.m <- y_trd.m + coef_reg[4+k]*D.m[,k]
    }
    y_trd <- coef_reg[1] + coef_reg[2]*t + coef_reg[3]*t^2 + coef_reg[4]*t^3 + y_trd.m
  }
...
### Predict for a PCS model under AR(1) and changepoints --- EDIT THID FUNCTION!!!
fct.PLTar <- function( fit, y, cp, n.ahead ) {     # y    : data
                                                   # cp   : changepoint chromosome (m; tau_1,...,tau_m)
  n <- length( y )                                 # n    : sample size
  m <- cp[1]                                       # m    : number of changepoints
