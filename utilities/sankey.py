...
def visualize_particles(num_of_particles, resampling_indices):
    ...
    positions[0] = np.arange(
        num_of_particles
    )  # Initial positions are just the particle indices
    ...
    for step in range(1, time_steps):
        for p in range(num_of_particles):
            plt.plot(
                [step - 1, step],
                [resampling_indices[step][p], positions[step, p]],
                linestyle="solid",
                color="darkcyan",
                markersize=0.1,
                linewidth=30 / num_of_particles,
            )
